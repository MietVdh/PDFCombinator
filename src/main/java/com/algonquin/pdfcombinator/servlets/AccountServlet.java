...
public class AccountServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		User user;
		//added try catch as appDAO changed and needs try catch
		try {
				...
				if (dao.getUserByEmail(newEmail) != null) {
					...
					try {
						URI verificationLink = new URI(
								"http://localhost:8080/PDFCombinator/verify?id=" + id + "&code=" + code);
					...
					System.out
							.println("Please paste this link in your browser to update your email address: \n" + link);
					message += "Please click the verification link we sent to you in order to complete updating your email address to "
							+ newEmail + "\n";
